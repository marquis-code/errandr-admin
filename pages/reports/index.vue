        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-50/50">
            <tr v-for="report in filteredReports" :key="report._id" class="hover:bg-gray-50/80 transition-colors group cursor-pointer" @click="viewReport(report)">
              <td class="py-4 px-2">
                <span class="text-xs font-bold text-parentPrimary uppercase">#{{ report._id.slice(-6) }}</span>
              </td>
      ...
      <template v-if="selectedReport">
        <div class="flex flex-col items-center justify-center py-6 border-b border-gray-100">
           ...
           <div class="flex flex-col items-center justify-center w-full mt-6 bg-gray-50 p-6 rounded-xl border border-gray-100/50">
             <span :class="statusBadge(selectedReport.status)" class="text-[11px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border mb-3">
               {{ selectedReport.status }}
             </span>
             <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Ticket #{{ selectedReport._id.slice(-6).toUpperCase() }}</p>
        ...
        <div class="py-6 space-y-6">
          <div class="grid grid-cols-2 gap-4">
             <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <p class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Reporter</p>
             ...
             <div class="bg-gray-50 p-4 rounded-2xl border border-gray-100" v-if="selectedReport.orderId">
                <p class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Related Order</p>
          ...
          <div>
            <h4 class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Description</h4>
          ...
          <div class="pt-6 border-t border-gray-100">
            <h4 class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-4">Resolution Note</h4>
            <textarea 
               v-model="adminNote" 
               rows="3" 
