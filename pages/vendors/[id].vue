...
const submitManualDebit = async () => {
  if (!debitAmount.value || debitAmount.value <= 0 || !vendor.value?.owner?._id) return;
  ...
  try {
    ...
    await GATEWAY_ENDPOINT_WITH_AUTH.post(`/wallets/admin/debit/${vendor.value.owner._id}`, {
