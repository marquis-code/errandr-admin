  ...
  <div class="space-y-10 animate-fade-in max-w-7xl mx-auto pb-20">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div
        ...
        :key="report._id"
        class="bg-white rounded-xl p-5 border border-gray-100 shadow-sm hover:shadow transition-all cursor-pointer group flex flex-col border-l-4"
      ...
      >
        ...
        <div class="flex items-center justify-between pt-4 border-t border-gray-100">
          <div class="flex items-center gap-2">
            <div class="w-6 h-6 rounded flex items-center justify-center bg-gray-100 text-gray-600 text-[10px] font-bold border border-gray-100">
    ...
    <Transition name="fade">
      <div v-if="selectedReport" class="fixed inset-0 z-50 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center p-4 transition-all" @click.self="selectedReport = null">
        <div class="bg-white rounded-xl p-0 w-full max-w-2xl relative animate-scale-in flex flex-col max-h-[90vh] overflow-hidden border border-gray-100 shadow-xl">
          <!-- Modal Header -->
          <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-white">
            ...
            <button @click="selectedReport = null" class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 hover:bg-rose-50 hover:text-rose-500 transition-colors border border-gray-100">
              <X class="w-4 h-4" />
            </button>
          </div>
          ...
          <div class="overflow-y-auto p-5 space-y-6 flex-1 custom-scrollbar bg-gray-50/30">
            <div class="space-y-2">
              ...
              <div class="text-xs text-gray-700 leading-relaxed font-medium bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            ...
            <div v-if="selectedReport.thread?.length" class="space-y-2">
              ...
              <div class="space-y-3">
                <div v-for="msg in selectedReport.thread" :key="msg.timestamp" class="flex flex-col" :class="msg.isAdmin ? 'items-end' : 'items-start'">
                  <div :class="msg.isAdmin ? 'bg-gray-900 text-white rounded-tr-sm shadow-sm' : 'bg-white text-gray-900 rounded-tl-sm border border-gray-100 shadow-sm'" class="max-w-[85%] px-4 py-2.5 rounded-xl">
          ...
          <div class="p-4 bg-white border-t border-gray-100 space-y-3">
            <textarea 
              ...
              class="w-full h-24 p-3 rounded-lg bg-gray-50 border border-gray-100 focus:ring-2 focus:ring-[#FF5C1A]/20 focus:border-[#FF5C1A] transition-all text-base font-medium resize-none placeholder:text-gray-400" 
            />
