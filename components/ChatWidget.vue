...
function getBubblePosition(message: ChatMessage) {
  if (message.senderType === 'customer' || message.senderType === 'guest' || message.senderType === 'admin') {
    return 'justify-end';
  }
...
function showSenderLabel(message: ChatMessage) {
  return message.senderType !== 'customer' && message.senderType !== 'guest' && message.senderType !== 'admin';
}
